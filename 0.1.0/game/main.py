...
class GameLoop:
    ...
    def battle(self):
        ...
        print()
        print(
            f'      {"~" * 10} {self.hero.name} vs {self.enemy.name} {"~" * 10}')
        print()
        print(f'                    Stage: {self.stage}                      ')
        print()
        if self.enemy_damage_increase != 0:
            print(
                f'     Enemy damage increased by: +{self.enemy_damage_increase}')

        if self.enemy_health_increase != 0:
            print(
                f'     Enemy health increased by: +{self.enemy_health_increase}')
        ...
        while self.hero.is_alive() and self.enemy.is_alive():
            ...
            if not self.hero.is_alive():
                ...
                print()
                print(
                    f'        {"~" * 10} {self.hero.name} has been defeated! 💀 {"~" * 10}')
                time.sleep(2)
                ...
                if self.hero.weapon is not None:
                    print()
                    print(
                        f'  Your weapon ({self.hero.weapon.name}) has been lost!')
                    print()
    ...
    def start_game(self):
        while True:
            ...
            if cutie.prompt_yes_or_no('     Do you want to play again?', default_is_yes=True, char_prompt=False):
                ...
                next_enemy = Enemy(name=new_enemy.name, health=new_enemy.health +
                                   self.enemy_health_increase, damage=new_enemy.damage + self.enemy_damage_increase)
